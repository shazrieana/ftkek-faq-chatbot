...
# 2. PAGE CONFIG & CUSTOM STYLES (To match the UMS colors)
st.set_page_config(page_title="FTKEK AI Assistant", layout="centered")

# Custom CSS to make the bubbles look like the UMS screenshot and fix the typing bar
st.markdown("""
    <style>
    /* Your existing chat bubble styles */
    .stChatMessage { 
    ...
    /* 1. PUSH THE TYPING CONTAINER UP FROM THE EDGE */
    .stChatInputContainer { 
        bottom: 20px !important;  /* Moves the input box up by 20 pixels */
        padding-left: 10px !important;
        padding-right: 10px !important;
    }
    
    /* 2. ADD SPARE SPACE AT THE BOTTOM OF THE SCROLL ZONE */
    /* This stops the final message from being blocked behind the typing bar */
...
# 5. DISPLAY MESSAGES (The "Bubble" look)
...
if prompt := st.chat_input("Type your message..."):
    ...
    # --- STATE BRANCH 3: Handle the PERSISTENT HOD menu choice if active (CRITICAL OVERLAP FIX) ---
    elif st.session_state.waiting_for_head_type:
        # Check for the word 'engineering' first, then break down the subset intent
        ...
        # Safe structural separation for standalone Technology selections
...
# --- STATE BRANCH 5: SBERT search with Confidence Threshold ---
        ...
        # Reset any active menu states when a general search runs
        st.session_state.waiting_for_programme_type = False
        st.session_state.waiting_for_dean_type = False
        st.session_state.waiting_for_head_type = False
        
        # Vectorize user input and run semantic search against your CSV corpus
        query_embedding = model.encode(prompt, convert_to_tensor=True)
        hits = util.semantic_search(query_embedding, corpus_embeddings, top_k=1)
        ...
        # The 60% confidence safety net
        if score >= 0.60:
            # If confidence is high enough, give the verified answer from your CSV row
            response = df.iloc[best_match_index]['Answer']
            
            # Smart Auto-Activation Listeners for menus based on row indexing
            if best_match_index == 36:
                st.session_state.waiting_for_programme_type = True
            elif best_match_index == 46:
                st.session_state.waiting_for_head_type = True
        else:
            # Local fallback message to prevent misleading information or hallucinations
            response = "Sorry, I don't have that answer, but you can refer to the FTKEK admin office at +606-270 2112 or email ftkek@utem.edu.my."
